...
**Home node** = identity (register / JWT). Default in a fresh install: `https://node-eu.wikitraveler.org`. **Data node** = regional facts via `/api/peers/resolve`. Federation stays invisible; uncovered listings show “No WikiTraveler coverage here.” ([RFC-0002](./rfcs/0002-global-hub-access.md))
